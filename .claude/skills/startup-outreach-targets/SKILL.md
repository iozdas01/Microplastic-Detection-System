...
6. **HARD RULE: the profile is written in English** (headline and role descriptions, or English listed under Languages). Check it on the live profile before every invite. A profile in another language fails, full stop: the contact cannot read the message and the co-founder cannot join the thread (founder rule, `copy-rules.md` LR-B31a).
