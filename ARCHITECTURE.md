...
#### Cards drive the search (founder decision, 2026-08-31)

**Queries are derived from what each method card needs to know, not from the belief.** The
card is read first; its evidence needs become the queries; the search fills them.

This reverses the previous design, where one neutral query plan was derived from the belief
and its boundaries, collected into a shared corpus, and every card then interpreted that
fixed corpus. The reversal was made for a reason that survived argument: **queries derived
from the belief inherit the founder's framing, while queries derived from a card inherit only
an analytical question.** "What job is being hired, and what is currently hired for it?" is
structurally neutral in a way that "is the design-to-machine handoff painful?" is not.

The old design was protecting against a real failure — a card that searches for what it needs
will find it, because someone on the internet is complaining about anything. That protection
is preserved by two rules rather than by freezing the corpus:

1. **Every card runs its counter-query.** Not optional, and recorded whether or not it
   returns anything. The JTBD card asks what is hired AND who says their current tool is
   fine. The predecessor card asks who tried and stalled AND **who tried and succeeded** —
   the omission that produced a graveyard-only reading on 2026-08-31, when Protolabs had been
   running the belief's own mechanism profitably since 1999 and was found only by accident.
2. **Every query and its result pool into one shared corpus file** that all cards read. Cards
   search independently; they do not each keep private evidence. When two cards disagree, the
   pooling is what makes it a real disagreement about interpretation rather than an artifact
   of them having looked at different things.

A third rule is about the founder, not the agent: **the derived query set is shown to the
founder before it runs.** On 2026-08-31 an unreviewed plan fired 288 searches in verbose
prose nobody would ever post, tripped engine rate-limiting, and returned nothing; the founder
corrected it in one message (shorter queries, and the professional forums — WOODWEB,
Practical Machinist — rather than Reddit, where the trades are hobbyists). Query review is
cheap and it is where the run's quality is actually set.

**The split that governs when search may be method-driven at all:** search is safe for facts
and dangerous for desirability. Whether Katerra went bankrupt, what Mozaik costs, whether a US
equivalent of Cutwrights exists — search cannot bias a bankruptcy date or a price page.
Whether a pain exists and how badly it is felt — search can absolutely bias that, which is
what the mandatory counter-query is for.
